...
namespace GoogleSheetsToUnity
{
    ...
    {   
        ...
        public static void GetPages(string sheetId, UnityAction<List<PageProperties>> callback) 
        { 
            ...
            if (Application.isPlaying)
            {
                new Task(ReadPages(request, callback));
            }
#if UNITY_EDITOR
            else
            {
                EditorCoroutineRunner.StartCoroutine(ReadPages(request, callback));
            }
#endif
        }
        ...
        static IEnumerator ReadPages(UnityWebRequest request, UnityAction<List<PageProperties>> callback) 
        {
            if (Application.isPlaying)
            {
                yield return new Task(CheckForRefreshToken());
            ...
            using (request)
            {
                ...
                SheetNamesValues rawData = JSON.Load(request.downloadHandler.text).Make<SheetNamesValues>();
                if (callback != null)
                {
                    callback(rawData.sheets.Select(o=> o.properties).ToList());
                }
            }
