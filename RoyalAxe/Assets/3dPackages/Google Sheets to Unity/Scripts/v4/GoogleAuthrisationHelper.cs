...
namespace GoogleSheetsToUnity
{
    public class GoogleAuthrisationHelper : MonoBehaviour
    {
        ...
        public static Action OnCanWorkEvent;

        public static bool GetTokenFlag { get; private set; }
        ...
        static IEnumerator GetToken(WWWForm f)
        {
            using (UnityWebRequest request = UnityWebRequest.Post("https://accounts.google.com/o/oauth2/token", f))
            {
                ...
                SpreadsheetManager.Config.gdr.nextRefreshTime = DateTime.UtcNow.AddSeconds(SpreadsheetManager.Config.gdr.expires_in).ToString(CultureInfo.InvariantCulture);
        ...
        {
            DateTime.TryParse(SpreadsheetManager.Config.gdr.nextRefreshTime,CultureInfo.InvariantCulture, DateTimeStyles.None, out var nextRefreshTime); 
            if (DateTime.UtcNow > nextRefreshTime)
            {
                ...
                using (UnityWebRequest request = UnityWebRequest.Post("https://www.googleapis.com/oauth2/v4/token", f))
                {
                    ...
                    { 
                        var errorResponse = JsonUtility.FromJson<GoogleErrorResponse>(request.downloadHandler.text);
                        Debug.LogWarning($"{errorResponse.error} : {errorResponse.error_description}");
                        Debug.LogWarning("Сделай коннект: Window -> GSTU -> Open Config");
                    }
                    else
                    {
                        ...
                        SpreadsheetManager.Config.gdr.nextRefreshTime = DateTime.UtcNow.AddSeconds(newGdr.expires_in).ToString(CultureInfo.InvariantCulture);

#if UNITY_EDITOR
                        EditorUtility.SetDirty(SpreadsheetManager.Config);
                        AssetDatabase.SaveAssets();
                        OnCanWorkEvent?.Invoke();
                        GetTokenFlag = true;
#endif
                    }
                }
